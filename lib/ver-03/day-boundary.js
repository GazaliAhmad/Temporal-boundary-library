...
const LEGACY_FIXED_TIME_OPTION_KEYS = new Set([
  "hour",
  "minute",
  "second",
  "millisecond",
 ...
 */
function throwIfLegacyFixedTimeBoundaryOptions(options) {
  if (!options || typeof options !== "object") {
    return;
  }

  const legacyKeys = Object.keys(options).filter((key) =>
    LEGACY_FIXED_TIME_OPTION_KEYS.has(key),
  );
  ...
  const formattedKeys = legacyKeys.map((key) => `"${key}"`).join(", ");

  throw new TypeError(
    `FixedTimeBoundaryStrategy no longer accepts legacy boundary keys (${formattedKeys}) in v3. Use { timeZone: "Asia/Singapore", boundaryTime: "06:00" } instead.`,
  );
}
 ...
 * @param {unknown} options
 */
function throwIfLegacyDailyBoundaryOptions(options) {
  if (!options || typeof options !== "object") {
    return;
  }

  if (
    typeof options.getBoundaryForDate === "function" &&
...
class FixedTimeBoundaryStrategy extends BoundaryStrategy {
   ...
   */
  constructor(options = {}) {
    throwIfLegacyFixedTimeBoundaryOptions(options);
    super(options);
...
class DailyBoundaryStrategy extends BoundaryStrategy {
   ...
   */
  constructor(options = {}) {
    throwIfLegacyDailyBoundaryOptions(options);
    super(options);
  ...
  getBoundaryForDate(date) {
    ...
    if (boundary instanceof Date || typeof boundary === "string" || typeof boundary === "number") {
      throw new TypeError(
